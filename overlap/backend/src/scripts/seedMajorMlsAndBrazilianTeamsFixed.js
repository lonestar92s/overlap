...
function generateApiId(teamName) {
...
async function createVenueForTeam(teamData, country, countryCode) {
    ...
    const newVenue = new Venue({
        ...
        surface: 'Natural grass', // Use valid enum value
        website: null
    });
...
async function createTeam(teamData, leagueId, venueId, country, countryCode) {
    ...
    if (existingTeam) {
        ...
        if (updated) {
            ...
            console.log(`👥 Team ${teamData.name} already exists and is complete`);
        }
        return existingTeam;
    }

    const newTeam = new Team({
        apiId: generateApiId(teamData.name), // Required field
        name: teamData.name,
        apiName: teamData.apiName,
...
async function seedMajorTeams() {
    ...
    try {
        ...
        for (const teamData of MLS_TEAMS) {
            ...
            const venueId = await createVenueForTeam(teamData, 'USA', 'US');
            const team = await createTeam(teamData, mlsLeague._id, venueId, 'USA', 'US');
            if (team) mlsSuccess++;
        }
        ...
        for (const teamData of BRAZILIAN_TEAMS) {
            console.log(`\n📍 Processing ${teamData.name}...`);
            const venueId = await createVenueForTeam(teamData, 'Brazil', 'BR');
            const team = await createTeam(teamData, brazilLeague._id, venueId, 'Brazil', 'BR');
