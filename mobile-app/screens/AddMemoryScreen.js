...
const AddMemoryScreen = () => {
  ...
  const [homeTeam, setHomeTeam] = useState({ name: '', logo: null, apiId: null });
  const [awayTeam, setAwayTeam] = useState({ name: '', logo: null, apiId: null });
  const [venue, setVenue] = useState({ name: '', city: '', country: '', coordinates: null });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const dateValue = matchDate || formatDateToLocalString(new Date());
      ...
      if (response.success) {
        ...
        Alert.alert('Saved', 'Memory created successfully', [
      ...
      >
        ...
        >
          ...
          <View style={styles.detailsCard}>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              accessibilityLabel={detailsExpanded ? 'Hide match details' : 'Add match details'}
              accessibilityRole="button"
            >
        ...
        <View style={styles.footer}>
          <TouchableOpacity
            ...
            accessibilityLabel={loading ? 'Saving memory' : 'Save memory'}
            accessibilityRole="button"
          >
            {loading ? (
